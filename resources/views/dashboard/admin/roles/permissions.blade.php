                        ...
                        <table class="table table-hover align-middle">
                            ...
                            <tbody>
                                @forelse($permissions as $group => $groupPermissions)
                                    ...
                                    @include('dashboard.admin.roles._permission_rows', ['permissions' => $groupPermissions, 'roles' => $roles])
                                @empty
                                    <tr>
...
<script>
    document.addEventListener('DOMContentLoaded', function() {
        ...
        // Resaltar la columna del rol si se especificó uno
        @if(isset($highlightRoleId))
            const roleId = '{{ $highlightRoleId }}';
            const roleHeader = document.querySelector(`th[data-role-id="${roleId}"]`);
            if (roleHeader) {
                // Hacer scroll a la columna
                roleHeader.scrollIntoView({ behavior: 'smooth', block: 'nearest', inline: 'center' });
                
                // Resaltar temporalmente
                roleHeader.classList.add('bg-warning', 'bg-opacity-25');
                setTimeout(() => {
                    roleHeader.classList.remove('bg-warning', 'bg-opacity-25');
                    roleHeader.classList.add('bg-warning', 'bg-opacity-10');
                }, 2000);
            }
        @endif
        
        // Manejar la generación de permisos
        const generateBtn = document.getElementById('generatePermissions');
        if (generateBtn) {
            generateBtn.addEventListener('click', function() {
                if (confirm('¿Estás seguro de que deseas generar los permisos automáticamente?\n\nEsta acción analizará todas las rutas de la aplicación y creará los permisos correspondientes.')) {
